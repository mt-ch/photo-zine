...
// PROTOTYPE — Variant D: Infinite grid. Full-viewport draggable canvas,
// photos tiled edge-to-edge and repeating in every direction. No fixed
// "page" — the theme/credits float as overlays on top of the canvas.
...
export function VariantD() {
  ...
  const rowStart = Math.floor((-offset.y - CELL) / CELL);
  const rowEnd = Math.ceil((-offset.y + viewport.height + CELL) / CELL);
  ...
  return (
    <div
      className="fixed inset-0 overflow-hidden bg-neutral-900 touch-none cursor-grab active:cursor-grabbing"
      {...bind}
    >
      <div
      ...
      <div className="pointer-events-none fixed left-4 top-4 text-white">
